...
export class Doctor extends React.Component {
    ...
    componentDidMount() {
        ...
        if(doctorID){
            axios.get('http://localhost:8762/doctor/id/' + doctorID)
                .then(response =>{
    ...
    updateDoctor = event => {
        ...
        axios.put("http://localhost:8762/doctor/update/"+this.state.id,DoctorConstant)
            .then(response=>{
                if(response.data != null){
                    alert("Updated!");
                }
    ...
    submitDoctor = event => {
        ...
        axios.post("http://localhost:8762/doctor/post/",DoctorConstant)
            .then(response=>{
                if(response.data != null){
